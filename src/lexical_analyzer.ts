...
/**
 * Separates words, expands {@link common_amh_abbreveations}, removes numbers, breaks up hyphenated words, and removes punctuation
 * @param corpus : Amharic text
 * @returns : Lexically analyzed Amharic text
 */

function lexAnalyze(corpus: string) {
  ...
  for (const key in common_amh_abbreveations) {
    ...
    corpus = corpus.replace(
      regex,
      `${
        common_amh_abbreveations[key as keyof typeof common_amh_abbreveations]
      }`
    )
  }
